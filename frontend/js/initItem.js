...
var newItem = function (title, content, urgency, lastModifiedTime, id) {
    ...
    var item =
        ...
        '<button class="btn btn-danger" id="delete_item_btn" style="float:right">Delete</button>' +
        '<button class="btn btn-info" id="edit_item_btn" style="float:right">Edit</button>';
    if (urgency != "finished")
        item += '<button class="btn btn-success" id="check_item_btn" style="float:right">Check</button>';
    item +=
        '</div>' +
        '</div>' +
        '</div>';
